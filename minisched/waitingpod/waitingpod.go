...
// Allow declares the waiting pod is allowed to be scheduled by a plugin.
...
func (w *WaitingPod) Allow(pluginName string) {
	...
	if timer, exists := w.pendingPlugins[pluginName]; exists {
		timer.Stop()
		delete(w.pendingPlugins, pluginName)
	}

	// Only signal success status after all plugins have allowed the pod
